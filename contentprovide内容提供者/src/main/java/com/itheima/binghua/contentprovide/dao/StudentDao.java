package com.itheima.binghua.contentprovide.dao;
...
import com.itheima.binghua.contentprovide.entities.Student;
...
public class StudentDao {
    ...
    private StudentSQLiteOpenHelper sso = null;

    public StudentDao(Context context) 
    {
        sso = new StudentSQLiteOpenHelper(context);
    ...
    public Student queryStudent (int id) {
        ...
        if (database.isOpen()) 
        {
            ...
            if (cursor !=null && cursor.moveToFirst()) {
                ...
                Student student = new Student(id1,name, 'F', score);
                database.close();
                return student;
            }
            database.close();
           ...
           Cursor cursor =database.rawQuery("select id,name,sex,score from Student;", null);
            if (cursor !=null && cursor.getCount() > 0) 
            {
                ...
                while (cursor.moveToNext())
                {
                    ...
                    studentList.add(new Student(id,name, 'M', score));
                }
                database.close();
